...
$(document).ready(function () {
    var mapWidth = $(".Map-Container").outerWidth() - $("#select1").outerWidth() - $("#select2").outerWidth() - 34;
    $("#container").css("width", mapWidth);

    $("#slide-btn").click(function () {
        if ($("#select2").css("display") == 'none') {
            mapWidth = $(".Map-Container").outerWidth() - $("#select1").outerWidth() - $("#select2").outerWidth() - 34;
            $("#container").animate({width: mapWidth}, 350);
            $("#select2").animate({width: 'toggle'}, 350);
            $("#slide-btn").html('<div class="iconfont icon-sanjiao1"></div>');
        } else {
            mapWidth = $(".Map-Container").outerWidth() - $("#select1").outerWidth() - 17;
            $("#select2").animate({width: 'toggle'}, 350);
            $("#container").animate({width: mapWidth}, 350);
            $("#slide-btn").html('<div class="iconfont icon-sanjiao"></div>');
        }
    })
})
...
$(document).ready(function () {
    ...
    var zans = '<div class="iconfont icon-circle zan"></div>';
    for (var i = 0; i < 2; i++) {
        zans += '<div class="iconfont icon-circle zan"></div>';
    }
    var grade = 7.5;
    if (grade > 9.0) {
